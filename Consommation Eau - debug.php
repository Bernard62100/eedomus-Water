...
//**********************************************************************************************************
// V2.1 : Script de suivi de la consommation en eau
//*************************************** ******************************************************************
...
if ($type_cumul) {
  if ($action == 'updatetarif' || $action == 'updateconso') {
    ...
    $preload = loadVariable('MYWATERGAPI_ABO_'.$api_compteur);
    if ($preload != '' && substr($preload, 0, 8) != "## ERROR") {
      // charge l'api du capteur tarif
      $api_abo = $preload;
      $preload = loadVariable('MYWATERG_ABO_'.$api_compteur);
      if ($preload != '' && substr($preload, 0, 8) != "## ERROR") {
        $abobase = $preload;
        $abo_ok = true;
        if (!strpos($abobase, ".")) {
    ...
    //      printf("-1#".'MYWATERGAPI_CPT_'.$api_compteur."\n");
    $preload = loadVariable('MYWATERGAPI_CPT_'.$api_compteur);
    //    printf("0#".$preload."\n");
    if ($preload != '' && substr($preload, 0, 8) != "## ERROR") {
  ...
  if ($action == 'updateconso') {
    ...
    //printf("valeur de mon compteur1 = ".$value."\n");
    $etat_compteur = $value['value'];
    //printf("valeur de mon compteur2 = ".$etat_compteur."\n");
    ...
    $preload = loadVariable('MYWATERG_LASTRELEVE_'.$api_compteur);
    //printf("2#".$preload."\n");
    if ($preload != '' && substr($preload, 0, 8) != "## ERROR") {
    ...
    $preload = loadVariable('MYWATERG_CPT_'.$api_compteur);
    if ($preload != '' && substr($preload, 0, 8) != "## ERROR") {
      $tab_cpt = $preload;
    } else {
      $tab_cpt['global'] = 0;
    ...
    $preload = loadVariable('MYWATERG_RELEVES_'.$api_compteur);
    if ($preload != '' && substr($preload, 0, 8) != "## ERROR") {
      $tab_releves = $preload;
    } else {
    ...
    $preload = loadVariable('MYWATERG_COUTS_'.$api_compteur);
    if ($preload != '' && substr($preload, 0, 8) != "## ERROR") {
      $tab_couts = $preload;
    } else {
    ...
    $prevannuel = "...";
    $preload = loadVariable('MYWATERG_PREV_'.$api_compteur);
    if ($preload != '' && substr($preload, 0, 8) != "## ERROR") {
      $prevannuel = $preload;
    }
...
if ($action == 'migrate') {
  ...
  $preload = loadVariable('MYWATERG_RELEVES');
  if ($preload != '' && substr($preload, 0, 8) != "## ERROR") {
    $tab_releves = $preload;
    if (array_key_exists($api_compteur, $tab_releves)) {
      ...
      $preload = loadVariable('MYWATERG_COUTS');
      if ($preload != '' && substr($preload, 0, 8) != "## ERROR") {
        $tab_couts = $preload;
      ...
      $preload = loadVariable('MYWATERG_CPT');
      if ($preload != '' && substr($preload, 0, 8) != "## ERROR") {
        $tab_cpt = $preload;
      ...
      $preload = loadVariable('MYWATERG_LASTRELEVE');
      if ($preload != '' && substr($preload, 0, 8) != "## ERROR") {
